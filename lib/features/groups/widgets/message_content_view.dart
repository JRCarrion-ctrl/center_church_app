...
extension FileExtension on String {
  ...
  bool get isDoc => ['doc', 'docx', 'ppt', 'pptx', 'xls', 'xlsx'].contains(extension);
...
class _MessageContentViewState extends State<MessageContentView> {
      ...
      children: [
        FutureBuilder<File?>(
          ...
          builder: (context, snapshot) {
            ...
            // 1. If cached, show the media/file locally
            if (isFileCached) {
              if (fileUrl.isImageOrGif) {
                return _buildCachedMediaView(file, fileUrl);
                 ...
                 return _buildFileCard(fileUrl, file);
              }
            }
            
            // 2. If NOT cached AND it's an image/GIF, SHOW CDN
            if (fileUrl.isImageOrGif) {
              return _buildNetworkMediaView(fileUrl);
            }

            // 3. Fallback: Not cached. If supported doc, tap to open directly via web viewer
            ...
            // 4. Generic file, not cached: tap to download
            return _buildFileActionCard(
              fileUrl, 
            ...
            child: SizedBox(
              ...
              child: Padding(
                ...
                child: LinkPreview(
                  ...
                  onLinkPreviewDataFetched: (data) => WidgetsBinding.instance.addPostFrameCallback((_) {
                  }),
                  text: previewUrl,
                ),
              ),
  ...
  Widget _buildFileActionCard(
    ...
    final icon = fileUrl.isVideo 
        ...
        : (fileUrl.isPdf ? Icons.picture_as_pdf : _getFileIcon(fileUrl.extension));
          ...
          child: Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    customText ?? "key_download_file".tr(args: [fileUrl.fileName]),
  ...
  Widget _buildCachedMediaView(File file, String fileUrl) {
    return GestureDetector(
      onTap: () => _showFullscreenMedia(context, file: file, url: fileUrl), 
      child: ClipRRect(
        borderRadius: BorderRadius.circular(16),
  ...
  Widget _buildFileCard(String fileUrl, File file) {
    final textColor = widget.isMe ? Colors.white : Theme.of(context).colorScheme.onSurface;
    final themeColor = Theme.of(context).brightness == Brightness.dark
        ? const Color.fromARGB(64, 255, 255, 255)
        : const Color.fromARGB(64, 0, 0, 0);
    ...
    return InkWell(
      onTap: () async {
        try {
          ...
          if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("key_file_open_failed".tr())));
        }
      },
      child: ClipRRect(
        borderRadius: BorderRadius.circular(16),
    ...
    return Linkify(
      ...
      onOpen: (link) async {
        ...
        if (await canLaunchUrl(uri)) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("key_173".tr())));
        }
      },
    );
  }

  // --- NEW: Universal Media Viewer ---
  void _showFullscreenMedia(BuildContext context, {File? file, required String url}) {
    ...
    if (url.isVideo) {
      viewerContent = Center(
        child: Material(
          color: Colors.transparent, // Ensures the background remains black
    ...
    showDialog(
      ...
      builder: (_) => Stack(
        children: [
          Container(
            color: Colors.black,
            padding: url.isPdf || url.isDoc ? const EdgeInsets.only(top: 80) : EdgeInsets.zero,
            child: viewerContent,
          ),
          Positioned(
            top: 30,
